    ...
    <div className="min-h-screen bg-navy-900 pt-20">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
          {[
            { label: "Total Portfolio Value", value: "$0.00", icon: <DollarSign />, change: "+0.0%" },
            { label: "Active Investments", value: "0", icon: <Briefcase />, change: "0" },
            { label: "Est. Annual Yield", value: "0.0%", icon: <TrendingUp />, change: "+0.0%" },
            { label: "Available Cash", value: "$0.00", icon: <Activity />, change: "Deposit" }
          ].map((stat, i) => (
            <div key={i} className="bg-navy-800 p-6 rounded-sm border border-white/5">
               <div className="flex justify-between items-start mb-4">
                 ...
                 <span className={`text-xs font-mono px-2 py-1 rounded ${i === 3 ? 'bg-gold-600/20 text-gold-500 cursor-pointer' : 'bg-emerald-500/10 text-emerald-500'}`}>
                   {stat.change}
                 </span>
               </div>
             ...
             <div className="flex justify-between items-center mb-6">
               ...
               <div className="flex gap-2">
                 {['1M', '3M', '6M', '1Y', 'ALL'].map(t => (
                   <button key={t} className="text-xs font-mono px-3 py-1 rounded bg-navy-900 text-slate-400 hover:text-white hover:bg-white/5 transition-colors">
                     {t}
                   </button>
                 ))}
               </div>
